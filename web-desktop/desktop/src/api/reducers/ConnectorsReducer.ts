...
import ConnectorAuth from "../../interfaces/connectors/ConnectorState";
import UpdateConnector from "../../interfaces/connectors/UpdateConnector";

const initialState = {
    data:{} as ConnectorAuth,
    status: "idle", 
    error : {},
  };
  ...
  export const getConnectors= createAsyncThunk(
    ...
    try{
      const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/connector/connectors/${id}/printers?skip=${skip}&limit=${limit}`,{
        headers: {
  ...
  export const updateConnector = createAsyncThunk(
    '/updateConnector',
    async ({ id, updateConnector, token }: { id: string | null | undefined, updateConnector: UpdateConnector | null | undefined, token: string | null | undefined }, thunkAPI) => {
      try {
        await axios.put(`${import.meta.env.VITE_SERVER_URL}/connector/${id}/printers`, updateConnector, { headers: { Authorization: `Bearer ${token}` } });
      } catch (error) {
        if (error instanceof AxiosError && error.response) {
          if (error.code === 'ERR_NETWORK') {
            return thunkAPI.rejectWithValue({message : 'Erreur de connexion: Veuillez vérifier votre connexion internet.'});
